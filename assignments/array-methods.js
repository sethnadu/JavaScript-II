...
// Problem 1
// Anybody with a large shirt got angry and took their donation back, figure out what the total donation's lost would be.

let people = runners.filter((size) => {
    return size.shirt_size == "L";
});

let totalDonationsLost = people.reduce((acc, currentValue) => {
    return acc += currentValue.donation;
}, 0);
console.log(totalDonationsLost);

// Problem 2
//List the people's first and last name who have small sized shirts
let totalPeople = runners.filter((small) => {
    return small.shirt_size == "S";
});

let totalPeopleSmall = totalPeople.map((names) => {
    return `${names.first_name} ${names.last_name}`
});

console.log(totalPeopleSmall);


// Problem 3
// List the people's id and last name who donated more than $150.
let donatedMore = runners.filter((more) => {
    return more.donation >= 150;
});

let donatedIDAndName = donatedMore.map((idandname) => {
    return `${idandname.id}, ${idandname.first_name} ${idandname.last_name}`
});

console.log(donatedIDAndName);